...
namespace webapi.Controllers
{
    ...
    public class ActivityDiscountsController : ControllerBase
    {
        ...
        private async Task<List<List<List<Discount>>>> GetBestDiscount(CartActivityDiscountsDto dto)
        {
            ...
            foreach (var discount in activityDiscounts)
            {
                ...
                if (isContain)
                {
                    ...
                    if (discount.ActivityDiscount.DiscountTypeName == "贈品")
                    {
                        otherProducts = items;
                    }
                    else
                    ...
                    {
                        var otherActivityDiscounts = activityDiscounts.Where(ad => ad.ActivityDiscount.AcitivityDetailId != discount.ActivityDiscount.AcitivityDetailId).ToList();
                        ...
                        for (int i = 0; i < otherActivityDiscounts.Count(); i++)
                        {
                            ...
                            if (isContainOther)
                            {
                                ...
                                if (otherDiscount.ActivityDiscount.DiscountTypeName == "贈品")
                                {
                                    anotherProducts = items;
                                }
                                else
                                {
                                    anotherProducts = otherProducts.Except(otherDiscount.DiscountProducts).ToArray();
                                }
        ...
        private async Task<List<Discount>> CartActivityDiscount(CartActivityDiscountsDto dto)
        {
            ...
            var items = dto.CartVM;
            var member = await _context.Members.Where(m => m.Id == memberId).FirstAsync();
        ...
        public async Task<ActionResult<List<Discount>>> GetProductDiscount(ProductActivityDiscountDto dto)
        {
            ...
            if (memberId != null) member = await _context.Members.FirstAsync(m => m.Id == memberId);
