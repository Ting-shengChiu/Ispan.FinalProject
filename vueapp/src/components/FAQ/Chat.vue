...
connection.on("messageReceived", (remoteUsername, message) => {
    const newMessage = {
        message,
...
connection.on("AdminAlive", () => adminAlive.value = true);
        ...
        <div class="chat-window d-flex flex-column h-100">
            ...
            <div class="chat-body">
                <v-container fluid class="pa-4 px-6 pb-12 h-100 overflow-y-auto" ref="chatwindow">
                    ...
                    <ChatMessage v-for="message in messages" :is-self="message.isSelf" :key="message.key" v-show="adminAlive">{{ message.message
                    }}</ChatMessage>
                </v-container>
            </div>
